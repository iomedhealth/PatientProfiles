...
If you are working with an IOMED database where both current events and medical history are stored in the same clinical tables, you can use the `filterCurrentEvents()` function to ensure PatientProfiles only uses current events when computing intersections.
